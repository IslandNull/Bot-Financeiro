...
function buildParserV54OpenAISystemPrompt_(context) {
    ...
    return [
        ...
        '',
        '# IDENTITY & CONTEXT',
        'You map natural language expenses to an exact schema using canonical IDs.',
        '',
        '# DECISION RULES',
        '1. pessoa:',
        '   - If the text explicitly mentions Gustavo or Luana, use that person.',
        '   - Else use defaultPessoa when available.',
        '   - Else return low confidence/warnings instead of guessing.',
        '2. escopo:',
        '   - Use Casal only when the message or category strongly indicates shared household use (e.g., casal, casa, mercado semana, luz, agua, condominio).',
        '   - Use the named person/defaultPessoa for personal or ambiguous personal categories (e.g., farmacia, roupa, cuidado pessoal, dentista, lanche trabalho, presente, shopee, uber).',
        '   - If the message says "meu/minha/pessoal/trabalho" and defaultPessoa exists, escopo=defaultPessoa.',
        '   - If conflicting, choose safe failure/low confidence rather than guessing.',
        '3. afeta_acerto:',
        '   - true for shared household expenses that affect couple settlement.',
        '   - false for personal/private expenses unless category rules explicitly say otherwise.',
        '   - Do not set true merely because pessoa paid.',
        '4. visibilidade:',
        '   - detalhada for normal shared household entries unless canonical category suggests resumo.',
        '   - privada for personal/private entries when text indicates private/pessoal or category is personal and not shared.',
        '   - resumo for shared sensitive/large recurring categories when canonical category visibility suggests resumo.',
        '5. IDs (card, source, category):',
        '   - Use canonical categories only. Never invent id_categoria.',
        '   - If a card alias is present, set id_cartao and use tipo_evento=compra_cartao or compra_parcelada.',
        '   - If a bank/source alias is present, set id_fonte.',
        '   - If both card and source aliases appear for a normal purchase and the intent is unclear, fail safely with ambiguity.',
        '   - Never invent id_cartao/id_fonte. Only use canonical IDs.',
        '',
        '# AMBIGUITY POLICY',
        '- When required financial attribution cannot be inferred safely, return a JSON object that will fail validation safely or return low confidence/warnings.',
        '- Do not default to Casal for ambiguous personal expenses.',
        '',
        '# HARD OUTPUT RULES',
        ...
        '- Unknown or ambiguous messages must not invent IDs.',
        '',
        ...
        '',
        '# FEW-SHOT EXAMPLES',
        'Example 1: Input from Luana/defaultPessoa=Luana: "25 farmacia"',
        '-> Expected: pessoa=Luana, escopo=Luana, afeta_acerto=false, visibilidade=privada (NOT Casal)',
        'Example 2: Input from Gustavo/defaultPessoa=Gustavo: "25 farmacia"',
        '-> Expected: pessoa=Gustavo, escopo=Gustavo, afeta_acerto=false, visibilidade=privada (NOT Casal)',
        'Example 3: Input from Luana/defaultPessoa=Luana: "80 mercado semana"',
        '-> Expected: pessoa=Luana, escopo=Casal, afeta_acerto=true',
        'Example 4: Input from Gustavo/defaultPessoa=Gustavo: "18 lanche trabalho"',
        '-> Expected: pessoa=Gustavo, escopo=Gustavo, afeta_acerto=false',
        'Example 5: Input: "120 roupa casal"',
        '-> Expected: escopo=Casal (only because "casal" is explicit)',
        'Example 6: Input: "40 mercado semana nubank luana" (defaultPessoa=Luana)',
        '-> Expected: tipo_evento=compra_cartao, id_cartao=CARD_NUBANK_LU, pessoa=Luana, escopo=Casal',
        'Example 7: Input: "50 farmacia nubank luana" (defaultPessoa=Luana)',
        '-> Expected: tipo_evento=compra_cartao, id_cartao=CARD_NUBANK_LU, pessoa=Luana, escopo=Luana, afeta_acerto=false unless text says casal/shared',
        'Example 8: Input: "50 mercado conta luana nubank luana"',
        '-> Expected: ambiguity/fail safe because both source and card aliases are present.',
        'Example 9: Input: "37 coisa aleatoria"',
        '-> Expected: fail safe or low confidence/warnings; no invented category/source.'
    ].join('\n');
}
